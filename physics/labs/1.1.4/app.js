(() => {
  ...
  function simulate(){
    ...
    const grouped=groupData(raw,k);drawContinuousHistogram(grouped,$('simChart'));const s=stats(grouped,k);$('simStatus').textContent=`Получено ${grouped.length} значений. Среднее ≈ ${format(s.mean,3)}, σ ≈ ${format(s.sigma,3)}.`;const typeNames={poisson:'Пуассон',exponential:'экспоненциальное',pareto:'Парето'};$('simCaption').textContent=`${typeNames[type] || type}, N=${N}, k=${k}`;
  }
